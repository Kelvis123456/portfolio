...
export const projects: Project[] = [
  ...
  },
  {
    ...
    tagline: {
      en: "A narrative detective game built to out-design a viral mobile hit",
      es: "Un juego narrativo de detective construido para superar en diseño a un hit móvil viral",
    },
    kind: "software",
    category: "game",
    status: "live",
    ...
    problem: {
      en: "ColdTrace, a mobile game built entirely around reading a stranger's simulated phone, went viral. I already had a detective game with three layers ColdTrace doesn't (a physical crime scene, an interrogation system, an evidence board) — the gap was that the phone/PIN/messaging mechanic ColdTrace built its whole identity on didn't exist here yet, and closing it had to be done without copying its name, icons, or any real messaging-app UI.",
      es: "ColdTrace, un juego móvil construido enteramente alrededor de leer el teléfono simulado de un desconocido, se volvió viral. Yo ya tenía un juego de detective con tres capas que ColdTrace no tiene (una escena física, un sistema de interrogatorio, un tablero de evidencia) — el hueco era que la mecánica de teléfono/PIN/mensajería sobre la que ColdTrace construyó toda su identidad no existía todavía aquí, y había que cerrarlo sin copiar su nombre, íconos, ni ninguna UI real de app de mensajería.",
    },
    solution: {
      en: "Verified the actual copyright situation first (the phone-reading subgenre predates ColdTrace by years — A Normal Lost Phone, Simulacra), then built a real digital-forensics layer with its own invented app ecosystem (ChatVía, Anotta, Vozal, NubePlus) inside an actual phone frame with alternating chat bubbles, plus a means/motive/opportunity accusation system with four distinct endings, a narrative tension mechanic that can permanently lock evidence if the player stalls, and a connectable evidence board — none of it bolted onto the old architecture; the same CaseEngine/EvidenceEngine/InterrogationEngine from the original build still drive it, extended, not replaced.",
      es: "Primero verifiqué la situación real de copyright (el subgénero de \"revisa el celular de alguien\" es anterior a ColdTrace por años — A Normal Lost Phone, Simulacra), y luego construí una capa real de forensia digital con su propio ecosistema de apps inventadas (ChatVía, Anotta, Vozal, NubePlus) dentro de un marco de teléfono real con burbujas de chat alternadas, además de un sistema de acusación por medios/móvil/oportunidad con cuatro finales distintos, una mecánica de tensión narrativa que puede bloquear evidencia para siempre si el jugador se demora, y un tablero de evidencia conectable — nada de esto se le pegó con cinta a la arquitectura vieja; el mismo CaseEngine/EvidenceEngine/InterrogationEngine del build original sigue impulsándolo, extendido, no reemplazado.",
    },
    architectureHighlights: {
      en: [
        ...
        "Zustand for global game state, Framer Motion for scene transitions, 378 Vitest tests including store-level audio side effects verified with spies",
      ],
      es: [
        ...
        "Zustand para el estado global del juego, Framer Motion para las transiciones de escena, 378 tests de Vitest incluyendo efectos secundarios de audio verificados con spies a nivel de store",
      ],
    },
    metrics: [
      { label: { en: "Cases", es: "Casos" }, value: "4" },
      { label: { en: "Tests", es: "Tests" }, value: "378 (Vitest)" },
    ],
    links: [
    ...
    gallery: [
      "/images/detective-game/digital-forensics.png",
      "/images/detective-game/connections.png",
      "/images/detective-game/interrogation.png",
      "/images/detective-game/crimescene.png",
      "/images/detective-game/menu.png",
    ],
    accentColor: "#8a6a3a",
  },
  {
